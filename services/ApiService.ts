...
import {setIsRefreshing, setTokens} from "@redux/sessionSlide";
...
axiosInstance.interceptors.request.use(
  (config: InternalAxiosRequestConfig) => {
    const access_token = store.getState().session.tokens?.access_token;
  ...
  async (error: AxiosError<ResponseData>) => {
    ...
    if (error.response && error.response.status === 401 && !store.getState().session.isRefreshing) {
      ...
      try {
        ...
        const tokens = await SessionService.refreshTokensAsync(store.getState().session.tokens!);
        await TokensHelper.setTokensAsync(tokens);
